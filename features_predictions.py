...
class DataNormalizer:
    ...
    def fix_numerical_distributions(self):
        # TODO: check those features: Phone_minutes_10_years, Avg_monthly_expense_on_pets_or_plants, Last_school_grades
...
class Network(nn.Module):
    ...
    def run(self):
        ...
        for epoch in range(EPOCHS):
            ...
            for batch in data_train:
                x_train, y_train = batch['inp'], batch['oup']
                loss = train(self, x_train, y_train, optm, criterion)
                epoch_loss += loss
            if epoch % 100 == 99:
...
def get_best_model():
    N, D_in, H, D_out = 16, 31, 64, 12
    models = [DynamicNet(D_in, H, D_out).run(), run_feed_forward_network(), Network().run()]
